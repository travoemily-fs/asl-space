...
router.post("/", uploadImage, planetCtlr.create); // create new
router.get("/:id(\\d+)", planetCtlr.show); // show one by id
router.patch("/:id(\\d+)", uploadImage, planetCtlr.update); // edit one
...
router.post("/:id(\\d+)", uploadImage, planetCtlr.update); // handles form editing
